...
public class UserDAO {
    ...
    private static final String GETSTORGE = "SELECT productID, name, price, quantity FROM tblProduct";
    ...
    public Cart getStorge() throws SQLException {
        Cart storge = new Cart();
        Connection conn = null;
        PreparedStatement ptm = null;
        ResultSet rs = null;
        try {
            conn = DBUtils.getConnection();
            if (conn != null) {
                ptm = conn.prepareStatement(GETSTORGE);
                rs = ptm.executeQuery();
                while (rs.next()) {
                    ...
                    storge.add(new Clothes(productID, name, price, quantity));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public boolean checkout(UserDTO user, Cart cart, double total) throws ClassNotFoundException, SQLException, NamingException {
        boolean check = false;
        check = insertOrder(user, total);
        int orderID = getOrderID();
        if (check) {
            if (orderID > 0) {
                check = insertOrderDetail(orderID, cart);
            } else {
                check = false;
            }
        }
    ...
    private boolean insertOrder(UserDTO user, double total) throws ClassNotFoundException, SQLException, NamingException {
        boolean check = false;
        Connection conn = null;
        PreparedStatement ptm = null;
        try {
    ...
    private boolean insertOrderDetail(int orderID, Cart cart) throws SQLException {
        boolean check = false;
        Connection conn = null;
        PreparedStatement ptm = null;
        try {
            ...
            if (conn != null) {
                for (Clothes cloth : cart.getCart().values()) {
                    ...
                    ptm.setDouble(3, cloth.getPrice()*cloth.getQuantity());
        ...
        try {
            ...
            if (conn != null) {
                for (Clothes cloth : cart.getCart().values()) {
                    ...
                    ptm.setInt(1, cloth.getQuantity());
                    ptm.setString(2,cloth.getId());
                    check = ptm.executeUpdate() > 0 ? true : false;
                    if (!check){
                        break;
